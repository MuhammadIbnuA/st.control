...
def filter_reviews_by_keywords_cosine(reviews, keywords):
...
def filter_reviews_by_keywords_jaccard(reviews, keywords, threshold=0.01):
    filtered_reviews = []
    similarity_scores = []
...
def main():
    ...
    similarity_measure = st.radio("Choose Similarity Measure:", ("Cosine Similarity", "Jaccard Similarity"), index=0)
    ...
    if app_id:
        ...
        if st.button("Filter Reviews"):
            if similarity_measure == "Cosine Similarity":
                ...
                reviews_with_keywords, similarity_scores = filter_reviews_by_keywords_cosine(reviews_content, keywords)
            else:
                # Filter reviews using Jaccard similarity
                reviews_with_keywords, similarity_scores = filter_reviews_by_keywords_jaccard(reviews_content, keywords)
